            ...
            <AppBar position="static" style={{ backgroundColor: "rgb(51, 53, 69)" }} >
                <Container maxWidth="xl">
                    <Toolbar disableGutters className='header_toolbar'>
                        ...
                        <Box sx={{ minWidth: 120 }} className='hide_box'>
                            <FormControl fullWidth>
                                <NativeSelect
                                    defaultValue={30}
                                >
                                    <option value={30}>Chandigarh</option>
                                    <option value={20}>Delhi-NCR</option>
                                    <option value={10}>Jaipur City</option>
                                </NativeSelect>
                            </FormControl>
                        </Box>
                        <Button variant="contained" color="error" className='sign_hide'>
                            Sign In
                        </Button>
                        <IconButton edge="start" color="inherit" aria-label="menu" sx={{ mr: 2 }}>
                            <MenuIcon className='menuicon-three-line' />
                        </IconButton>
                    </Toolbar>
                </Container>
            </AppBar>
            <AppBar className='secound-navbar' position="static" style={{ backgroundColor: "rgb(31, 37, 51)" }}>
                <Link className='movie-name-link' style={{ textDecoration: 'none' }} to='/'>
                    Home
                </Link>
                <Link className='movie-name-link' style={{ textDecoration: 'none' }} to='/Movie'>
                    Movies
                </Link>
                <Link className='movie-name-link' style={{ textDecoration: 'none' }} to='/TopRatedMovie'>
                    Top Rated Movie
                </Link>
                <Link className='movie-name-link' style={{ textDecoration: 'none' }} to='/PopularTVShow'>
                    <span>TV Show</span>
                </Link>
            </AppBar>
            <style jsx>{`
            :global(.MuiToolbar-root .MuiToolbar-regular .css-r6ewbb-MuiToolbar-root) {
                display: flex;
